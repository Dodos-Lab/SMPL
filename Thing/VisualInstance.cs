...
";
	...
	internal abstract class VisualInstance : ThingInstance
	{
		...
		public void SetCustomEffect(Thing.CodeGLSL shaderCode)
		{
			custom = shaderCode;
			Effect = Thing.Effect.Custom;
		...
		private Thing.CodeGLSL custom;
		internal readonly static SortedDictionary<int, List<VisualInstance>> visuals = new();
		internal readonly static Dictionary<Thing.Effect, Thing.CodeGLSL> shaders = new()
		{
			{ Thing.Effect.None, default },
			{ Thing.Effect.ColorFill, new()
			{
				FragmentUniforms = @"
...
FinalColor = vec4(color, FinalColor.a);", } },
			{ Thing.Effect.ColorReplaceLight, new()
			{
				FragmentUniforms = @"
...
for(int i = 0; i < 10; i++)
	if (ColorEqualsColor(FinalColor, Colors[i], Ranges[i]))
		FinalColor *= ColorTints[i];"
			...
			{ Thing.Effect.ColorsReplace, new()
			{
				FragmentUniforms = @"
uniform vec4 Colors[10];
...
FinalColor = GetPixelColor(Texture, TextureCoords);"
		...
		internal void SetShader(Thing.CodeGLSL shaderCode)
		{
			shader?.Dispose();
			shader = null;
